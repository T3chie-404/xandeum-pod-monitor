...
const LOCALHOST_PORTS = [22, 80, 3000, 4000, 6000, 7000];
const PUBLIC_UDP_PORTS = [5000, 9001];
const PUBLIC_TCP_PORTS = [22, 80, 3000, 4000, 6000];
...
class NetworkManager {
  ...
  static async checkLocalhostPort(port) {
    try {
      const { stdout } = await execPromise(`netstat -tln 2>/dev/null | grep -E ":${port} "`);
      const isPublic = stdout.includes(`0.0.0.0:${port}`);
      const isLocal = stdout.includes(`127.0.0.1:${port}`) || isPublic;
  ...
  static async checkPublicPort(ip, port, protocol = "tcp") {
    try {
      let command;
      if (protocol === "udp") {
        // UDP: check if listening on 0.0.0.0 (all interfaces = public)
        command = `netstat -uln | grep -E "0\.0\.0\.0:${port} "|grep -q udp`;
      } else {
