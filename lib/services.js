...
class ServiceManager {
  ...
  static async controlService(serviceName, action) {
    ...
    // Prevent self-restart/stop (causes disconnection)
    if (serviceName === 'xandeum-pod-monitor' && (action === 'restart' || action === 'stop')) {
      return {
        success: false,
        service: serviceName,
        action: action,
        error: 'Cannot restart/stop monitor service from within itself. Use: sudo systemctl ' + action + ' xandeum-pod-monitor.service'
      };
    }
