...
class SystemMonitor {
  ...
  static async getHealthStatus() {
    ...
    // Check CPU
    if (stats.cpu.usage >= 91) {
      issues.push("High CPU usage");
      score -= 20;
    } else if (stats.cpu.usage >= 71) {
      issues.push("Moderate CPU usage");
      score -= 10;
    }
    
    // Check memory
    if (stats.memory.percentage >= 91) {
      issues.push("High memory usage");
      score -= 20;
    } else if (stats.memory.percentage >= 76) {
      issues.push("Moderate memory usage");
      score -= 10;
    }
    
    // Check disk
    if (stats.disk.percentage >= 91) {
      issues.push("Low disk space");
      score -= 30;
    } else if (stats.disk.percentage >= 81) {
